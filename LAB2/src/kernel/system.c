...
FORWARD _PROTOTYPE( int do_setpriority, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getpriority, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETPRIORITY: r = do_setpriority(&m); break;
	    case SYS_GETPRIORITY: r = do_getpriority(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  if (prio == 1) {
  	rpc->base_priority = 50;
  } else {
  	rpc->base_priority = 10;
  }
  prio = 1 - prio;
  rpc->cur_priority = rpc->base_priority;
...
PRIVATE int do_setpriority(m_ptr)
register message *m_ptr;
{
	struct proc *pp;
	...
	pp->base_priority = m_ptr->m1_i2;
	pp->cur_priority = m_ptr->m1_i2;
	return 0;
}

PRIVATE int do_getpriority(m_ptr)
register message *m_ptr;
{
	struct proc *pp;
