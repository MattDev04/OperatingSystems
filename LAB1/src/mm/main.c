...
PUBLIC int do_getmaxchild() {
    int max_children_count = 0;
    int process_with_max_children = -1;
    int i, j, child_count;
    for (i = 0; i < NR_PROCS; i++) {
        if (!(mproc[i].mp_flags & IN_USE)) continue;
        child_count = 0;
        for (j = 0; j < NR_PROCS; j++) {
            if (mproc[j].mp_flags & IN_USE && mproc[j].mp_parent == i) {
                child_count++;
            }
        }
        if (child_count > max_children_count) {
            max_children_count = child_count;
            process_with_max_children = mproc[i].mp_pid;
        }
    }
    mproc[who].mp_reply.m1_i1 = process_with_max_children;
    mproc[who].mp_reply.m1_i2 = max_children_count;
    return 0;
}

PUBLIC int do_getmaxdescendants(void) {
    int i;
    int result_pid = 0;
    int result_count = -1;
    int excluded_pid = mm_in.m1_i1;
    int descendants_counter[NR_PROCS] = {0};
    int id;
    for (i = 0; i < NR_PROCS; i++) {
        if (mproc[i].mp_flags & IN_USE) {
            int parent_id = mproc[i].mp_parent;
            while (mproc[parent_id].mp_pid != 0) {
            	++descendants_counter[parent_id];
        	id = parent_id;
        	parent_id = mproc[id].mp_parent;
            }
        }
    }
    for (i = 0; i < NR_PROCS; i++) {
        if (mproc[i].mp_pid != excluded_pid && result_count < descendants_counter[i]) {
            result_pid = mproc[i].mp_pid;
            result_count = descendants_counter[i];
        }
    }
    mproc[who].mp_reply.m1_i1 = result_pid;
    mproc[who].mp_reply.m1_i2 = result_count;
    return 0;
}
